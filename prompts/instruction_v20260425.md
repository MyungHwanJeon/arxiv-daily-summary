...
## [랩 ROI 키워드 — 8개 버킷]
1. **Object Recognition** - 6D Object Pose Estimation, Transparent Object
2. **Robot Manipulation** - Manipulation, VLA, Diffusion Policy, 
3. **3D/Scene** — 3D Gaussian Splatting, NeRF, SLAM, scene reconstruction, neural implicit
4. **Robot Learning** — VLA models, imitation learning, sim2real, teleoperation, dexterous, humanoid
5. **Autonomous Driving** — end-to-end driving, BEV perception, motion planning, closed-loop eval
6. **Foundation Models** — VLM, hallucination, multimodal alignment, VLM reasoning
7. **Generation** — diffusion, video gen, world models, 3D generation
8. **Efficiency/Systems** — sparse MoE, efficient attention, KV cache, on-device
9. **Embodied AI** — navigation, ObjectNav, instruction following, memory-augmented policies
10. **Safety/Alignment** — VLA safety, RL safety, OOD detection
...
- 스타일 기준 파일: https://raw.githubusercontent.com/MyungHwanJeon/arxiv-daily-summary/main/posts/template.html
  → WebFetch로 가져와 CSS/구조/배지 스타일을 그대로 복제. 일관성 유지가 중요.

**홈으로 돌아가기 버튼 (필수)**:
- 모든 개별 날짜 페이지 상단(제목/meta 섹션 근처)과 하단 footer 근처,
  총 2곳에 "← 홈으로" 또는 "🏠 전체 목록으로" 버튼을 배치.
- 링크 대상: `https://MyungHwanJeon.github.io/arxiv-daily-summary/` (index).
...
## [배포 — GitHub Pages]
1. 작업 디렉토리: `{WORKDIR}`
2. 디렉토리 없으면: `gh repo clone MyungHwanJeon/arxiv-daily-summary {WORKDIR}`
...
🔗 <https://MyungHwanJeon.github.io/arxiv-daily-summary/posts/YYYY-MM-DD.html|전체 리포트 보기>
📊 *주간 한 줄 요약*
...
- index.html에서 구독 안내 블록과 `<link rel="alternate">` autodiscovery 포함.
  외부인은 RSS 리더에 `https://MyungHwanJeon.github.io/arxiv-daily-summary/feed.xml`
...
- 템플릿:
  ...
  🔗 <https://MyungHwanJeon.github.io/arxiv-daily-summary/posts/YYYY-MM-DD-weekly.html|전체 회고 보기>
